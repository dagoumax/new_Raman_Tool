...
将 JPG 光谱图像转换为数组，支持行分组、列合并和拉曼位移校准。
"""
...
def read_jpg(
    ...
    if arr.ndim == 1:
        ...
        return Spectrum(
            raman_shift=raman, intensity=arr,
            filename=filepath.name,
            metadata={"filepath": str(filepath.absolute()), "format": "JPG", "image_shape": original_shape},
        )
    ...
    if rows == 1:
        ...
        if calibration:
            raman = calibration[0] * raman + calibration[1]
        return Spectrum(
            raman_shift=raman, intensity=arr[0, :],
            filename=filepath.name,
            metadata={"filepath": str(filepath.absolute()), "format": "JPG", "image_shape": original_shape},
        )
    if cols == 1:
    ...
    return Spectrum(
        raman_shift=raman_shift, intensity=intensity,
        filename=filepath.name,
        metadata={
            "filepath": str(filepath.absolute()), "format": "JPG",
            "image_shape": original_shape, "row_groups": row_groups,
            "col_merge": col_merge, "selected_rows": final_rows,
            "output_cols": final_cols,
            "individual_rows": row_spectra if row_spectra else None,
            "total_rows": rows, "calibration": calibration,
        },
    )
